...
namespace world {
	bool load();
	bool has_player_online(uint32_t id);
	World get_by_id(int id);
	extern int self;
};
...
public:
	...
	static void removeCoins(uint32_t accountId, int32_t coins);
	static void onTransactionCompleted(uint32_t accountId, int32_t coins, const std::string& description);
};
